import { MemberAdded as MemberAddedEvent, MessageAdded, ViolationAdded } from "../generated/TazMessage/TazMessage"
import { MemberAdded, Message, Violation } from "../generated/schema"

export function handleMemberAdded(event: MemberAddedEvent): void {
    let entity = new MemberAdded(event.transaction.hash.toHex() + "-" + event.logIndex.toString())
...
export function handleMessageAdded(event: MessageAdded): void {
    let message = new Message(event.params.messageId.toString())
