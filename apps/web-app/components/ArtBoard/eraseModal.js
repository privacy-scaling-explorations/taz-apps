...
import WarningTriangle from "../svgElements/WarningTriangle"

const EraseModal = ({ eraseModalIsOpen, acceptEraseModal, cancelEraseModal }) => (
    <Transition appear show={eraseModalIsOpen} as={Fragment}>
        <Dialog as="div" className="relative z-40" onClose={cancelEraseModal}>
            <Transition.Child
                as={Fragment}
                ...
                <div className="flex min-h-full items-center justify-center p-4 text-center">
                    ...
                    >
                        <Dialog.Panel className="w-10/12 max-w-md transform overflow-hidden rounded-lg border-brand-blue border-2 bg-white text-left align-middle shadow-xl transition-all">
                            <div className="p-4 mb-1 bg-brand-beige border-b-2 border-brand-blue justify-between flex">
                                <div className="flex items-center">
                                    <WarningTriangle />
                                    <p className="text-brand-red font-bold pl-2"> Heads Up</p>
                                </div>
                                <button
                                    type="button"
                                    className="flex items-center focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-beige2"
                                    onClick={cancelEraseModal}
                                >
                                    <svg
                                        className="mr-2"
                                        width="18"
                            ...
                            <div className="p-4">
                                <p className="mb-7 flex justify-center text-sm text-brand-blue">
                                    You can submit one tile to this canvas. You can choose a new tile any time before
                                    you submit, but changing tiles will erase anything you've already drawn.
                                </p>
                                <div className="flex justify-center mb-3 mt-7">
                                    <button
                                        type="button"
                                        className="inline-flex justify-center rounded-full border border-transparent bg-brand-yellow px-6 py-1 font-bold text-brand-black hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-beige focus-visible:ring-offset-2"
                                        onClick={acceptEraseModal}
                                    >
                                        Erase my last drawing
                                    </button>
                                </div>
                            </div>
                        </Dialog.Panel>
