...
const MyProfilePage = ({ events, sessions }: Props) => {
  ...
  useEffect(() => {
    if (userSessions.length > 0) {
      const eventsName = userSessions
        .map((item) => item.events)
        .map((event) => event.name.replace("\n", ""));
      const uniqueValues = eventsName.filter(
  ...
  const sortByDate = userSessions.sort(
    (a, b) => (new Date(a.startDate) as any) - (new Date(b.startDate) as any)
  );

  const filteredSessions =
    selectedOpt.length > 0
      ? sortByDate.filter((item) =>
          selectedOpt.includes(item.events.name.replace("\n", ""))
        )
      : sortByDate;
  return (
    <BaseTemplate>
              ...
              <div className="flex gap-2 flex-col justify-center items-start">
                {tickets &&
                  tickets.map((item, index) => (
                    <div
                      key={index}
                      className="flex items-center gap-1 cursor-pointer w-auto"
                    >
                      <NextImage
                        src={"/vector-ticket-black.svg"}
                        width={14}
                        height={12}
                      />
                      <a
                        onClick={() =>
                          downloadFile(item.pdf_link)
                        }
