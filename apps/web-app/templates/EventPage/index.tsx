...
const EventPage = ({ event, sessions }: Props) => {
  ...
  const formattedDate = `${startDate.format("MMM D")}-${endDate.format(
    "D, YYYY"
  )}`;
  ...
  for (
    let date = filterSince;
    date <= filterAfter;
    date.setDate(date.getDate() + 1)
  ) {
    const option = moment(new Date(date))
      .add(1, "day")
  ...
  const filteredSessions =
    selectedOptions.length !== 0
      ? sessions.filter((item) => {
          const sessionDate = moment(new Date(item.startDate))
            .add(1, "day")
            .format("dddd, MMMM Do, YYYY");
        ...
        <div className="flex flex-col md:flex-row w-full justify-start bg-white rounded-[8px] h-[682px]">
          ...
          <div className="flex flex-col w-full md:w-2/6 pl-5 pr-20">
            <div className="flex my-5 w-full">
              <h1 className="text-black text-[52px] font-[600]">{`${event.name.substring(
                0,
                30
              )}...`}</h1>
            </div>
            <div className="flex flex-col w-full gap-4">
              ...
              <h1>
                Join us for presentations, workshops, and roundtables to discuss
                beginner ZK, ZK for biohackers, new proving systems and more.
              </h1>
            </div>
            <div className="flex flex-col mt-10 gap-5">
              ...
              <div className="flex flex-wrap">
                {event.organizers?.map((organizer, idx) => (
                  <div
                    className="flex gap-2 bg-gray-200 rounded-[4px] px-3 py-1"
                    key={idx}
                  >
