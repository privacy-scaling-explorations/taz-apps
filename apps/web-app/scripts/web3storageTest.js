...
web3StorageClient.put(dataFileArrayForServingImage, { wrapWithDirectory: false }).then((dataCid) => {
    const imagelUri = "https://" + dataCid + ".ipfs.dweb.link"
    console.log("Uri for serving image: ", imagelUri)
})

web3StorageClient.put(dataFileArrayForServingString, { wrapWithDirectory: false }).then((dataCid) => {
    const imagelUri = "https://" + dataCid + ".ipfs.dweb.link"
