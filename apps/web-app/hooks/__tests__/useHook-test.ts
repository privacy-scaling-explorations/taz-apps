...
const useHookTest = async () => {
    const SUBGRAPH_URL = "https://api.thegraph.com/subgraphs/name/semaphore-protocol/goerli"
    const postData = {
        query: `
            {
              members(first: 200 
    ...
    const newMemberList = membersList.map(({ identityCommitment }) => identityCommitment)
