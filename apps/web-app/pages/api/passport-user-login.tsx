    ...
    try {
      ...
      if (signIn.data?.user) {
        const updatePublicUser = await supabase
          .from("users")
          .update({ uui_auth: signIn.data.user.id })
          .eq("email", email);
        console.log("update pub", updatePublicUser)
      ...
      if (signIn.error) {
        console.log(
          "Sign in error received (this is expected if first time signing in): ",
          signIn.error
        );
        ...
        if (signUpResponse.data?.user) {
          ...
          const publicUserResponse = await supabase
            .from("users")
            .select("id")
            .eq("email", email);
          ...
          // Otherwise, add the user profile
            ...
            const addPublicUser = await supabase
              ...
              .insert({
                userName: name,
                email,
                uui_auth: signUpResponse.data.user.id,
              });
